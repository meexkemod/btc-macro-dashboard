...
with st.expander("🔍 Debug Data"):
    st.write("Candlestick", df_k.head())
    st.write("Open Interest", df_oi.head())
    st.write("Funding Rate", df_f.head())
    st.write("Order Book - Bids", bids.head())
    st.write("Order Book - Asks", asks.head())
...
if not df_k.empty:
    ...
    axs[0, 0].text(0.5, 0.5, "Data kosong", ha='center', va='center')
axs[0, 0].grid(True)
...
if not df_oi.empty:
    ...
    axs[0, 1].text(0.5, 0.5, "Data kosong", ha='center', va='center')
axs[0, 1].grid(True)
...
if not df_f.empty:
    ...
    axs[1, 0].text(0.5, 0.5, "Data kosong", ha='center', va='center')
axs[1, 0].grid(True)
...
if not bids.empty and not asks.empty:
    ...
    axs[1, 1].text(0.5, 0.5, "Data kosong", ha='center', va='center')
axs[1, 1].grid(True)

# 5. CVD
if not df_k.empty and "CVD" in df_k.columns:
    axs[2, 0].plot(df_k["Time"], df_k["CVD"], color="purple")
    axs[2, 0].set_title("CVD (Delta Volume)")
else:
    axs[2, 0].text(0.5, 0.5, "Data kosong", ha='center', va='center')
axs[2, 0].grid(True)
